  ...
  rtsHandler.doWifiConnect(
    ...
    15).then(function(msg) {
      if(msg.value.wifiState == 2 || msg.value.wifiState == 1) {
        ...
        try { scanForWifi(); } catch(_e) { setPhase("containerWifiConfig"); }
...
function scanForWifi() {
  rtsHandler.doWifiScan().then(function(m) {
    ...
    try { $("#txtWifiSsid").removeClass("readonly"); $("#txtWifiSsid").prop("readonly", false); } catch(_e) {}
    setPhase("containerWifiConfig");
    try { pterm_print("wifi scan timed out; enter ssid manually (use 2.4ghz)."); newLine(); } catch(_e) {}
  });
}
...
function updateStatusBox(m) {
  ...
  var _st = (m && m.value) || {};
  if(_st.wifiState == 1 || _st.wifiState == 2) {
    $("#vecInfoWifi").removeClass("vec-hidden");
    try {
      var _ssid = RtsCliUtil.convertHexToStr(_st.wifiSsidHex || "");
      $("#vecStatusSsid").html(_ssid == null ? (_st.wifiSsidHex || "") : _ssid);
    } catch(_e) { try { $("#vecStatusSsid").html(_st.wifiSsidHex || ""); } catch(_e2) {} }
  } else {
    $("#vecInfoWifi").addClass("vec-hidden");
  }
  try { $("#vecInfoEsn").html(_st.esn || ""); } catch(_e) {}
  try {
    var _ver = _st.version || "";
    var _parts = _ver.split("_os");
    firmware = (_parts.length > 1 ? _parts[1] : _ver).split("-")[0] || _ver;
  } catch(_e) { firmware = (_st && _st.version) || firmware || ""; }
  try { $("#vecInfoBuild").html(firmware); } catch(_e) {}
  try { robotSoftware = (_st.version || "").split("-")[0]; } catch(_e) {}
  $("#vecStatusTitle").html(vecBle.bleName);

  var _hasOwner = _st.hasOwner, _cloudAuthed = _st.isCloudAuthed;
  if(_version == 2 || _version == 3 || (!_hasOwner || _cloudAuthed)) {
...
function HandleHandshake(version) {
  ...
  rtsHandler.onEncryptedConnection(function() {
    ...
    var _statusTries = 0;
    var _afterStatus = function(m) {
      try { updateStatusBox(m); } catch(_e) { console.log("updateStatusBox failed (non-fatal):", _e); }
      var _st = (m && m.value) || {};
      ...
      if(_st.wifiState == 1 || _st.wifiState == 2) {
        ...
        try {
          rtsHandler.doWifiScan().then(function() {
            connectToWifi(_wifiCredentials.ssid, _wifiCredentials.pw, _wifiCredentials.auth);
          }, function() { connectToWifi(_wifiCredentials.ssid, _wifiCredentials.pw, _wifiCredentials.auth); });
        } catch(_e) { connectToWifi(_wifiCredentials.ssid, _wifiCredentials.pw, _wifiCredentials.auth); }
      }
      else {
        try { scanForWifi(); }
        catch(_e) {
          try { setPhase("containerWifiConfig"); } catch(_e2) { setPhase("containerDiscover"); }
        }
      }
      try {
        if(_statusInterval != null) { clearInterval(_statusInterval); }
        _statusInterval = setInterval(function() {
          try { rtsHandler.doStatus().then(function(mm) { try { updateStatusBox(mm); } catch(_e) {} }, function() {}); } catch(_e) {}
        }, 5000);
      } catch(_e) {}
    };
    var _doStatusOnce = function() {
      rtsHandler.doStatus().then(_afterStatus, function() {
        _statusTries++;
        if(_statusTries < 3) { _doStatusOnce(); return; }
        try { setPhase("containerWifiConfig"); } catch(_e) { setPhase("containerDiscover"); }
        try { pterm_print("status timed out 3x; continuing to manual wifi entry. if wifi join fails, use custom ssid + 2.4ghz."); newLine(); } catch(_e) {}
      });
    };
    _doStatusOnce();
